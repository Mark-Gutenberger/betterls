...
nerdfont.cpp (c) 2022
Desc: description
Created:  2022-05-12T21:50:35.737Z
Modified: 2022-05-14T01:53:20.128Z
*/
...
#define CHECK(predicate)                                                  \
	do {                                                                  \
		if (!report_check(__FILE__, __LINE__, #predicate, (predicate))) { \
			C4_DEBUG_BREAK();                                             \
		}                                                                 \
	} while (0)
#endif
...
void sample_quick_overview() {
	...
	// Iterating:
	{
		...
		{
			size_t count = 0;
			for (size_t child_id = tree.first_child(root_id); child_id != ryml::NONE;
		...
		{
			size_t count = 0;
			for (size_t child_id = tree.first_sibling(bar_id); child_id != ryml::NONE;
				 child_id = tree.next_sibling(child_id))
				CHECK(tree.key(child_id) == expected_keys[count++]);
		}
	}
	...
	root.append_child() << ryml::key("newkeyval (serialized)")
						<< "shiny and new (serialized)";  // serializes and assigns the serialization
...
void sample_parse_reuse_parser() {
	...
	auto champagnes = parser.parse_in_arena(
		"champagnes.yml", "[Dom Perignon, Gosset Grande Reserve, Ruinart Blanc de Blancs, Jacquesson 742]");
...
void sample_fundamental_types() {
	...
	CHECK(tree.arena() == "abcde0101234-45-56-67-70x10.1240.23410.inf");
	CHECK(tree.to_arena(dinf) == ".inf");
	CHECK(tree.arena() == "abcde0101234-45-56-67-70x10.1240.23410.inf.inf");
	CHECK(tree.to_arena(-finf) == "-.inf");
	CHECK(tree.arena() == "abcde0101234-45-56-67-70x10.1240.23410.inf.inf-.inf");
	CHECK(tree.to_arena(-dinf) == "-.inf");
	CHECK(tree.arena() == "abcde0101234-45-56-67-70x10.1240.23410.inf.inf-.inf-.inf");
	CHECK(tree.to_arena(fnan) == ".nan");
	CHECK(tree.arena() == "abcde0101234-45-56-67-70x10.1240.23410.inf.inf-.inf-.inf.nan");
	CHECK(tree.to_arena(dnan) == ".nan");
	CHECK(tree.arena() == "abcde0101234-45-56-67-70x10.1240.23410.inf.inf-.inf-.inf.nan.nan");
...
#undef _c4emscripten_alt
		...
		// --------------------------------------------------------------
		{
			C4_SUPPRESS_WARNING_CLANG_WITH_PUSH(
				"-Wcast-align")	 // we're casting the values directly, so alignment is strictly respected.
...
void sample_base64() {
	...
	text_and_base64 cases[] = {
		...
		{{"These violent delights have violent ends..."},
		 {"VGhlc2UgdmlvbGVudCBkZWxpZ2h0cyBoYXZlIHZpb2xlbnQgZW5kcy4uLg=="}},
		{{"How now, my love?"}, {"SG93IG5vdywgbXkgbG92ZT8="}},
		{{"Why is your cheek so pale?"}, {"V2h5IGlzIHlvdXIgY2hlZWsgc28gcGFsZT8="}},
		{{"How chance the roses there do fade so fast?"},
		 {"SG93IGNoYW5jZSB0aGUgcm9zZXMgdGhlcmUgZG8gZmFkZSBzbyBmYXN0Pw=="}},
		{{"Belike for want of rain, which I could well beteem them from the tempest of my eyes."},
		 {"QmVsaWtlIGZvciB3YW50IG9mIHJhaW4sIHdoaWNoIEkgY291bGQgd2VsbCBiZXRlZW0gdGhlbSBmcm9tIHRoZSB0ZW1wZXN0IG9mIG15IGV5"
		  "ZXMu"}},
	};
...
- bat
	...
	// emit a full tree as json
	{
		...
		CHECK(
			ryml::to_csubstr(s) ==
			R"(["a","b",{"x0": 1,"x1": 2},{"champagne": "Dom Perignon","coffee": "Arabica","more": {"vinho verde": "Soalheiro","vinho tinto": "Redoma 2017"},"beer": ["Rochefort 10","Busch","Leffe Rituel"]},"foo","bar","baz","bat"])");
	}
...
'far': 'a long long way to go'
	...
	CHECK(
		...
		R"({"doe": "a deer, a female deer","ray": "a drop of golden sun","me": "a name, I call myself","far": "a long long way to go"})");
...
struct ErrorHandlerExample {
	...
	void on_error(const char* msg, size_t len, ryml::Location loc) {
		throw std::runtime_error(ryml::formatrs<std::string>("{}:{}:{} ({}B): ERROR: {}", loc.name, loc.line, loc.col,
															 loc.offset, ryml::csubstr(msg, len)));
	}

	// bridge
	ryml::Callbacks callbacks() { return ryml::Callbacks(this, nullptr, nullptr, ErrorHandlerExample::s_error); }
...
struct GlobalAllocatorExample {
	...
	void* allocate(size_t len) {
		...
		if (C4_UNLIKELY(alloc_size > memory_pool.size())) {
			std::cerr << "out of memory! requested=" << alloc_size << " vs " << memory_pool.size() << " available"
					  << std::endl;
			std::abort();
		}
		return ptr;
	...
	static void s_free(void* mem, size_t len, void* this_) { return ((GlobalAllocatorExample*)this_)->free(mem, len); }

	// checking
	~GlobalAllocatorExample() { check_and_reset(); }
...
struct PerTreeMemoryExample {
	...
	// checking
	~PerTreeMemoryExample() { check_and_reset(); }
	void check_and_reset() {
		std::cout << "size: alloc=" << alloc_size << " dealloc=" << dealloc_size << std::endl;
		std::cout << "count: #allocs=" << num_allocs << " #deallocs=" << num_deallocs << std::endl;
		CHECK(num_allocs == num_deallocs);
