...
void mom() {
	...
	if (myfile.is_open()) {
		...
		std::cout << "that's okay, make a file `test.json` in the locatino you run this executable.\n";
	}
};
...
int main(int, char*[]) {
	...
	assert(document.IsObject());  // Document is a JSON value represents the root of DOM. Root can be either an object
								  // or array.
	...
	// This version of SetString() needs an allocator, which means it will allocate a new buffer and copy the the string
	// into the buffer.
	Value author;
	{
		char buffer2[10];
	...
	assert(author.IsNull());  // Move semantic for assignment. After this variable is assigned as a member, the variable
							  // becomes null.
